...
<label {{ $attributes }} for="{{ $questionNumber }}_{{ $optionNumber }}" class="bg-gray-100 w-full px-4 h-[38px] md:h-[45px] flex items-center gap-3 rounded-md shadow-sm cursor-pointer hover:bg-gray-200 transition">
    <input 
        ...
        {{ $pendingAnswer ==  $option['value'] || old('questions.' . $questionNumber) == $option['value'] ? 'checked' : '' }}
    />
    <div data-role="option-icon" class="{{ $pendingAnswer ==  $option['value'] || old('questions.' . $questionNumber) == $option['value']  ? 'opacity-100' : 'opacity-0' }}">
