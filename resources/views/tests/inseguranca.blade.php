...
<x-layouts.app>
    <main class="w-screen h-screen flex flex-col gap-5 items-center justify-center">
        <div class="bg-white rounded-md w-full max-w-screen-md min-h-2/4 max-h-3/4 shadow-md p-10 flex flex-col items-center justify-center gap-8">
            ...
            <form action="{{ route('test.submit', 'inseguranca')}}" id="test-form" class="flex-1 overflow-auto w-full space-y-5" method="post">
                @csrf
            ...
            <div class="flex items-center justify-between w-full">
                <a href="{{ route('test', 'pressao-por-resultados') }}" class="px-4 py-1 bg-transparent border border-rose-700 rounded-md text-md font-medium text-rose-700 hover:bg-rose-700 hover:text-white transition">
        ...
        <footer>    
            <p class="text-xs text-zinc-800">Baseado no Job Insecurity Scale (JIS) [De Witte, 2000] e ISO 45003:2021</p>
        </footer>
