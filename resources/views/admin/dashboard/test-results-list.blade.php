    ...
    <div class="w-screen h-screen flex overflow-hidden pt-16 md:pt-0">
        ...
        <div class="flex-1 overflow-auto p-4 md:p-8 flex flex-col items-start justify-start gap-6">
            ...
            <div class="w-full flex items-center gap-4">
                <div class="bg-white/25 flex-1 px-6 py-2 rounded-md shadow-md">
            ...
            <div data-role="filter-modal" class="hidden z-50 left-0 top-0 fixed w-screen h-screen bg-gray-800/30 px-4 flex items-center justify-center">
                <div class="w-full max-w-[360px] bg-gray-100 py-8 px-4 rounded-md flex flex-col items-center gap-8">
                    ...
                    <x-form class="w-full flex flex-col gap-4 items-center">
                        ...
                        {{-- <x-form.input-text name="department" placeholder="Setor" />TODO: Select --}}
                        {{-- <x-form.input-text name="occupation" placeholder="Cargo" />TODO: Select --}}
            ...
            <div class="w-full flex flex-col gap-4 items-end">
                <div class="w-full space-y-2" data-role="tests-list">
                    <div class="grid grid-cols-6 bg-gray-100 px-4 py-2 w-full rounded-md shadow-md">
                        <span data-role="th" onclick="reorderTable(event, 0)" class="text-base font-semibold text-left col-span-2">Nome</span>
                        <span data-role="th" onclick="reorderTable(event, 1)" class="text-base font-semibold text-left">Idade</span>
                        <span data-role="th" onclick="reorderTable(event, 2)" class="text-base font-semibold text-left">Setor</span>
                    ...
                    <div class="space-y-2" data-role="tbody">
                        @foreach ($testStatsList as $testStats)
                            <a
                                data-role="tr"
                                href="{{ route('user.info', $testStats['userId']) }}"
                                class="
                                    px-4 py-2 w-full rounded-md shadow-md bg-gradient-to-b from-[#FFFFFF25] grid grid-cols-6
                                ...
                                ">
                                <span data-role="td" class="col-span-2">{{ $testStats['name'] }}</span>
                                <span data-role="td">{{ $testStats['age'] }}</span>
                                <span data-role="td">{{ $testStats['department'] }}</span>
                                <span data-role="td">{{ $testStats['occupation'] }}</span>
                                <span data-value="{{ $testStats['testSeverityColor'] }}" data-role="td">{{ $testStats['testSeverityTitle'] }}</span>
                            </a>
                        @endforeach
                    </div>
                </div>
